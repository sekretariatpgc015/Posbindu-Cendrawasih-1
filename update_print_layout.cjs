...
// 1. Wrap the main component to apply `print:hidden`
content = content.replace(
  '<div className="space-y-6 pb-12" id="form-ptm-container">', 
  '<div className="relative">\n      <div className="space-y-6 pb-12 print:hidden" id="form-ptm-container">'
);

// 2. We need to add the PrintLayout and modal at the end before the final `);` of the return
const modalAndPrintHTML = `
      {/* Print Preview Modal */}
...
if (insertionPoint !== -1) {
  content = content.slice(0, insertionPoint) + '</div>\n' + modalAndPrintHTML + '}\n';
}

// 3. We previously inserted the modal with my previous script! Wait. Did we?
// Yes, I did `node get_modal.cjs` which injected the modal inside the main div!
// I need to reverse that or just checkout the file again. Wait, git is not available.
// I will just read the file, and do string manipulation carefully.
