...
import { Save, RefreshCw, UserCheck, HeartPulse, Search, Info, ChevronRight, FileText, Download, AlertTriangle, Filter } from 'lucide-react';
...
export default function FormPTMView({ wargaList, kunjunganList, onSaveKunjungan, onNavigateToDashboard }: FormPTMViewProps) {
  ...
  const exportKunjunganPDF = (data: Kunjungan[], fileName: string) => {
    if (data.length === 0) return;
    const doc = new jsPDF('landscape');
    doc.setFontSize(14);
    doc.text('Laporan Rekapitulasi Kunjungan Posbindu PTM', 14, 15);
    const tableData = data.map((k, index) => [
      index + 1, k.tanggal, k.nama, k.jenisKelamin === 'Laki-laki' ? 'L' : 'P',
      k.usia, k.rt, `${k.tdSistolik}/${k.tdDiastolik}`, k.gds || '-', k.chol || '-', k.au || '-'
    ]);
    autoTable(doc, {
      startY: 20,
      head: [['No', 'Tanggal', 'Nama', 'L/P', 'Usia', 'RT', 'TD', 'GDS', 'CHOL', 'AU']],
      body: tableData, theme: 'grid', styles: { fontSize: 8 }, headStyles: { fillColor: [79, 70, 229] }
    });
    doc.save(`${fileName}.pdf`);
  };

  const exportHipertensiPDF = (data: Kunjungan[], fileName: string) => {
    if (data.length === 0) return;
    const doc = new jsPDF('landscape');
    doc.setFontSize(14);
    doc.text('Daftar Warga Risiko Hipertensi Posbindu PTM', 14, 15);
    const tableData = data.map((k, index) => {
      const sys = k.tdSistolik, dia = k.tdDiastolik;
      let category = 'Hipertensi Ringan';
      if (sys >= 160 || dia >= 100) category = 'Hipertensi Derajat 2 (Berat)';
      else if (sys >= 140 || dia >= 90) category = 'Hipertensi Derajat 1';
      else category = 'Pre-Hipertensi / Perbatasan';
      return [index + 1, k.tanggal, k.nama, k.jenisKelamin === 'Laki-laki' ? 'L' : 'P', k.usia, k.rt, `${k.tdSistolik}/${k.tdDiastolik}`, category];
    });
    autoTable(doc, {
      startY: 20,
      head: [['No', 'Tanggal', 'Nama', 'L/P', 'Usia', 'RT', 'TD', 'Kategori']],
      body: tableData, theme: 'grid', styles: { fontSize: 8 }, headStyles: { fillColor: [220, 38, 38] }
    });
    doc.save(`${fileName}.pdf`);
  };
          ...
          <div className="p-6 border-b border-slate-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => exportKunjunganPDF(filteredKunjungan, 'data_kunjungan_posbindu')}
          ...
          <div className="p-6 border-b border-red-50 bg-red-50/20 flex flex-col gap-4">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto shrink-0">
                <button
                  type="button"
                  onClick={() => exportHipertensiPDF(filteredHypertensionKunjungan, 'skrining_hipertensi_posbindu')}
                  ...
                  title="Unduh laporan PDF Hipertensi"
                >
                  <FileText className="w-3.5 h-3.5" />
                  PDF
                </button>
